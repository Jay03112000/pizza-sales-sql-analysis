...
SELECT MAX(price) AS expensive_pizza FROM pizzas;

-- Identify the most common pizza size ordered.
SELECT pizza_type_id,COUNT(size) as common_size FROM pizzas
GROUP BY pizza_type_id
ORDER BY COUNT(size) DESC LIMIT 3;
...
SELECT 
category ,
COUNT(name) as Total_Types_of_Pizza
FROM pizza_types
GROUP BY 1;
...
CONCAT(Round((SUM(p.price*od.quantity)/
			(SELECT SUM(p.price*od.quantity)
				FROM pizzas AS p JOIN order_details AS od
					ON od.pizza_id=p.pizza_id))*100,2),'%') AS Revenue_Percentage
